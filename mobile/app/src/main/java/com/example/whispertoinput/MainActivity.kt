...
val ALT_THEME = booleanPreferencesKey("alt-theme")
...
class MainActivity : AppCompatActivity() {
            ...
            return CoroutineScope(Dispatchers.Main).launch {
                ...
                spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(parent: AdapterView<*>, view: View?, pos: Int, id: Long) {
                        ...
                        // Auto-save: write to DataStore immediately (no Apply needed)
                        isDirty = true
                        CoroutineScope(Dispatchers.Main).launch { apply() }
                    }
    ...
    /** Boolean setting backed by a SwitchCompat toggle */
    inner class SettingSwitch(
        ...
        override suspend fun apply() {
            if (!isDirty) return
            val newValue: Boolean = findViewById<SwitchCompat>(viewId).isChecked
            writeSetting(preferenceKey, newValue)
            isDirty = false
        }
    }
            ...
            return CoroutineScope(Dispatchers.Main).launch {
                ...
                spinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
                    override fun onItemSelected(parent: AdapterView<*>, view: View?, pos: Int, id: Long) {
                        if (!setupSettingItemsDone) return
                        isDirty = true
                        // Auto-save dropdown selection immediately
                        CoroutineScope(Dispatchers.Main).launch { apply() }
                        // Deal with individual spinner
                        if (parent.id == R.id.spinner_speech_to_text_backend) {
                            // Backend change also populates text fields — enable Apply for those
                            btnApply.isEnabled = true
    ...
    private fun setupSettingItems() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val settingItems = arrayOf(
                ...
                SettingSwitch(R.id.switch_auto_recording_start, AUTO_RECORDING_START, true),
                ...
                SettingSwitch(R.id.switch_alt_theme, ALT_THEME, false),
            )
            val btnApply: Button = findViewById(R.id.btn_settings_apply)
            ...
            btnApply.setOnClickListener {
                CoroutineScope(Dispatchers.Main).launch {
                    ...
                    // Save any remaining dirty text field settings
                    for (settingItem in settingItems) {
                        settingItem.apply()
                    }

                    // Test API connection
                    val endpoint = findViewById<EditText>(R.id.field_endpoint).text.toString()
                    val apiKey = findViewById<EditText>(R.id.field_api_key).text.toString()

                    if (endpoint.isNotBlank()) {
                        try {
                            // Test connection: any HTTP response means server is reachable
                            ...
                            val msg = when {
                                code in 200..299 -> "Connection successful!"
                                code == 401 -> "Server reachable, but API key rejected (401)"
                                code == 403 -> "Server reachable, but access denied (403)"
                                else -> "Server reachable (HTTP $code)"
                            }
                        ...
                        Toast.makeText(this@MainActivity, "Endpoint is empty", Toast.LENGTH_LONG).show()
                    }
                }
            }
        ...
        findViewById<View>(R.id.link_website)?.setOnClickListener {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://stmna.io")))
        }
        findViewById<View>(R.id.link_x)?.setOnClickListener {
            startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://x.com/STMNA_")))
        }
    }
}
